  ...
  Variants {
    ...
    PanelWindow {
      ...
      Process {
        ...
        stdout: StdioCollector {
          onStreamFinished: {
            ...
            cacheService.hasFfmpeg = root.hasFfmpeg;
          ...
          Keys.onPressed: event => {
            ...
            if (event.key === Qt.Key_Tab) {
              ...
              if (wallpaperModel.folders.length > 0) {
                const next = idx < wallpaperModel.folders.length - 1 ? idx + 1 : 0;
                switchFolder(wallpaperModel.folders[next]);
              }
              event.accepted = true;
              return;
            }
            ...
            if (event.key === Qt.Key_Backtab) {
              const idx = wallpaperModel.folders.indexOf(wallpaperModel.currentFolder);
              if (wallpaperModel.folders.length > 0) {
                const next = idx > 0 ? idx - 1 : wallpaperModel.folders.length - 1;
                switchFolder(wallpaperModel.folders[next]);
              }
              event.accepted = true;
              return;
      ...
      function applyWallpaper(path) {
        const scriptPath = Qt.resolvedUrl("./scripts/wallpaper.sh").toString().slice(7);
        const cmd = [
          "bash", "-c",
