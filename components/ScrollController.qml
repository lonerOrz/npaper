...
Item {
    ...
    Timer {
        ...
        onTriggered: {
            if (root.isKeyScrolling && root.keyScrollDirection !== 0 && root.count > 0) {
                ...
                if (root.keyScrollDirection === -1) nextIdx = Math.max(0, currentIdx - step)
    ...
    // Public Methods
    function scrollLeft() { _handleInput(-1) }
    function scrollRight() { _handleInput(1) }
    function fastScrollLeft() { _handleInput(-1, 5) }
    function fastScrollRight() { _handleInput(1, 5) }
    function scrollTo(idx) { root.scrollTarget = Math.max(0, Math.min(idx, root.count - 1)) }
    function random() { if (root.count > 0) root.scrollTarget = Math.floor(Math.random() * root.count) }
    ...
    function _handleInput(direction, step = 1) {
        if (root.keyScrollDirection !== direction) {
            ...
            const maxIdx = root.count - 1
            const currentIdx = Math.round(root.scrollTarget)
            if (direction === -1) root.scrollTarget = Math.max(0, currentIdx - step)
            else root.scrollTarget = Math.min(maxIdx, currentIdx + step)
